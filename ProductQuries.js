...
// List the four product which are grater than 500 in price 
db.products.find({ product_price : { $gt : 500 }}).sort({product_price:1}).limit(4).pretty();

// Find the product name and product material of each products
db.products.find({}, { _id:0, product_price : 1, product_material : 1 }).pretty();
...
// Find products which contain product color indigo  and product price 492.00
db.products.find({ $or:[{ product_price: { $eq : 492.00} }, { product_color : "indigo" }]}).pretty();
